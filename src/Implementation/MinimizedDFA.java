...
public class MinimizedDFA {
    ...
    public HashMap<Integer, HashMap<String, Integer>> getMinimizedDFATable () throws IOException {
        MutableGraph g = mutGraph("ejemplo").setDirected(true);

        for(Map.Entry<Integer, HashMap<String, Integer>> entry : this.minimizedDFATable.entrySet()){
            MutableNode nodo = mutNode("q"+ entry.getKey().toString());
            for(Map.Entry entry1 : entry.getValue().entrySet()){

                //nodo.addLink((entry1.getValue().toString());
                LinkSource from = nodo;
                LinkSource to = mutNode( "q" + entry1.getValue().toString());
                nodo.links().add(from.linkTo((LinkTarget) to).with(Label.of(entry1.getKey().toString())));
            }
            g.add(nodo);
            //g.add(mutNode( entry.getKey().toString()));
        }
        ...
        for(Map.Entry<Integer, HashMap<String, Integer>> entry : this.minimizedDFATable.entrySet()){
            ...
            for(Map.Entry entry1 : entry.getValue().entrySet()) {
               ...
               // g.add(mutNode( entry.getKey().toString()).addLink(mutNode( entry1.getValue().toString()) ));

            }

        }
        Graphviz.fromGraph(g).width(500).render(Format.PNG).toFile(new File("ex1m.png"));
